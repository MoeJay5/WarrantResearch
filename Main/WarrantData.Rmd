...
```{r}
ggplot() + geom_bar(data = dt_warrants, aes(x = 'warrants', fill = sex), position = 'fill')+ 
  labs(title = "Types of Crimes")
```
...
```{r}
ggplot() +
  geom_bar(data = dt_warrants, aes(x = type, fill = sex), color = "black", position = 'dodge', alpha = .90) +
  ggtitle('Issued Warrants by Gender') + xlab("Crime Type") + ylab("Warrants Amount") + labs(fill ='Sex') + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```